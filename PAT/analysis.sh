...
pcaps=(*.pcap)
if [[ ${#pcaps[@]} -eq 0 ]]; then
  echo "Error: no .pcap files found in current directory." >&2
  exit 1
fi
...
# Unique IP-addresses
uniq_ips () {
  tshark -r "$1" -T fields -e ip.dst -e ip.src 2>/dev/null >> "$2.uniq_ip.log"
}
...
for filecap in "${pcaps[@]}"; do
  ...
  # Wait until all processes are finished before moving to next pcap
  wait
